...
This document describes how to implement a **camera module** (imaging source) for the X-ray acquisition application. Camera modules provide the connection to a detector and deliver raw frames to the main app, which handles display, dark/flat correction, banding, dead pixels, and export.
...
- A **camera module** is a loadable Python package under `machine_modules/` that:
  - Exposes frame dimensions and builds its **Connection** UI (e.g. Connect/Disconnect).
  - Implements **acquisition only**: when the user clicks Start, the main app calls into the module, which runs a worker thread, acquires frames, and hands each frame to the app via **`gui.api.submit_frame(frame)`**. Dark and flat reference capture are **not** done by the camera; they are done by the **dark_correction** and **flat_correction** alteration modules.

Only one camera module is active at a time. The app loads it at startup if the corresponding setting is enabled (e.g. `load_hamamatsu_c7942_module`).
...
The main app builds the **Start** mode combo from **`get_acquisition_modes()`**. When the user clicks Start, the selected **mode_id** is passed to your worker via **`api.get_acquisition_mode()`**. Your camera module implements **only** these four modes:
...
**Dark** and **flat** reference capture are **not** camera modes; they are implemented by the **dark_correction** and **flat_correction** modules. When your worker finishes (any mode), call **`api.set_acquisition_idle()`** and clear progress.
...
Modules are **discovered automatically** from the **`machine_modules/`** folder. You do **not** need to add your module to **`gui.py`** or the Settings window.

- **Discovery:** The app uses **`machine_modules.registry`** to find all subpackages (e.g. **`faxitron`**, **`hamamatsu_c7942`**, **`open_image`**) and reads **`MODULE_INFO`** and **`get_setting_keys()`** from each.
- **In your module’s `__init__.py`:**
  - **`MODULE_INFO`** (dict): **`display_name`**, **`description`**, **`type`** (`"camera"` or **`"machine"`**), **`default_enabled`** (bool). For **`type == "camera"`** add **`camera_priority`** (int; higher = preferred when multiple camera modules are enabled).
...
- **Build UI:** **`_build_ui()`** picks the enabled **camera** module with highest **`camera_priority`**, calls **`get_frame_size()`** and **`build_ui()`** for it. For **machine** modules, it calls **`build_ui()`** for each enabled one.

See **`machine_modules/registry.py`** and any existing module (e.g. **`hamamatsu_c7942`**, **`faxitron`**) for **`MODULE_INFO`** and **`get_setting_keys()`**.

---

## 8. Reference implementation

- **`machine_modules/hamamatsu_c7942/`** and **`machine_modules/asi_camera/`** are reference camera modules:
...
Use either as the template for a new camera module.

---

## 9. Checklist for a new camera module

- [ ] Package under **`machine_modules/<module_name>/`** with **`__init__.py`**.
...
- [ ] **`MODULE_INFO`** with **`display_name`**, **`description`**, **`type": "camera"`**, **`default_enabled`**, **`camera_priority`** (higher = preferred when multiple cameras enabled).
...
└───────────────────────────────┬─────────────────────────────────┘
        ...
        ▼                                                ▼
┌─────────────────────────────┐              ┌──────────────────────┐
│  machine_modules.           │              │  Placeholder:        │
│  hamamatsu_c7942            │              │  "No camera module   │
...
This contract keeps the main app a generic X-ray acquisition shell and keeps all hardware-specific logic in the camera module.
