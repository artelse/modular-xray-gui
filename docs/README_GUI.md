...
The main application is a **generic X-ray acquisition shell**. It provides the viewport, acquisition controls, dark/flat correction, banding and dead-pixel correction, and export. It does **not** talk to hardware directly; imaging and machine hardware are provided by **loadable modules** under `machine_modules/`.
...
- **UI build:** `_build_ui()` creates the main window, texture, control panel, and calls into the **selected camera module** (frame size, Connection UI) and each **enabled machine module** (Faxitron, HV supply, etc.). Settings checkboxes for modules come from the **registry** (no hardcoding in gui.py).
...
- The **camera module** runs a worker thread that submits each frame via **`api.submit_frame(frame)`**. When the worker finishes it calls **`api.set_acquisition_idle()`**.
...
| **`_start_acquisition(mode)`** | Optional beam supply turn-on + wait (unless **`workflow_keep_beam_on`**); clear buffer; set acq_mode and start camera module. |
...
- **Camera and supply modules:** [machine_modules/MODULES_OVERVIEW.md](machine_modules/MODULES_OVERVIEW.md)
- **Camera module contract (detailed):** [machine_modules/README_CAMERA_MODULES.md](machine_modules/README_CAMERA_MODULES.md)
- **Per-module docs:** [machine_modules/&lt;name&gt;/README.md](machine_modules/) (e.g. ct_capture, asi_camera, esp_hv_supply).
