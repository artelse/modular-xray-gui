...
Saves raw 16-bit frame as TIFF (for PixInsight etc.) via -o/--output.
"""
...
# Full resolution: 2400x2400 (requires 16MB PSRAM on Teensy)
# 12-bit packed: 2400x2400x1.5 = 8,640,000 bytes
FRAME_WIDTH = 2400
FRAME_HEIGHT = 2400
...
def get_state(handle):
    ...
    state, row, col, stop_reason, dma_wait, overhead_us, dma_ok, dma_timeout = struct.unpack("<BIIBIIII", dat)
    ...
    print(f"State: {state_names.get(state, state)}, row={row}, col={col}, "
          ...
          f"dma_wait={dma_wait}, overhead_max={overhead_us}us, DMA ok={dma_ok}, timeout={dma_timeout}")
...
def save_tiff(pixels, path):
    """Save unpacked 16-bit frame as TIFF (raw 0-4095). Prefer tifffile for PixInsight-compatible headers."""
    import numpy as np
    frame = np.asarray(pixels, dtype=np.uint16).reshape((FRAME_HEIGHT, FRAME_WIDTH))
    try:
        import tifffile
        tifffile.imwrite(path, frame, photometric="minisblack", compression=None)
        print(f"Saved 16-bit TIFF: {path} ({FRAME_WIDTH}x{FRAME_HEIGHT})")
    except ImportError:
        from PIL import Image
        img = Image.fromarray(frame, mode="I;16")
        img.save(path, compression=None)
        print(f"Saved 16-bit TIFF: {path} ({FRAME_WIDTH}x{FRAME_HEIGHT}) [PIL]")


def main():
    parser = argparse.ArgumentParser(description="C7942 frame acquisition test; save raw 16-bit TIFF.")
    parser.add_argument("-o", "--output", default="frame.tif", help="Output TIFF path (default: frame.tif)")
    parser.add_argument("--no-save", action="store_true", help="Do not save TIFF")
    ...
    if frame_data:
        ...
        pixels = analyze_frame(frame_data)
        if pixels is not None and len(pixels) == FRAME_WIDTH * FRAME_HEIGHT and not args.no_save:
            out = args.output
            if not out.lower().endswith((".tif", ".tiff")):
                out = out + ".tif"
            save_tiff(pixels, out)
        elif pixels is not None and len(pixels) != FRAME_WIDTH * FRAME_HEIGHT and not args.no_save:
            print(f"Skipping TIFF save (incomplete frame: {len(pixels)} pixels)")
